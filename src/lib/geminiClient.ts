...
} from "@google/genai"; // Import necessary types
...
async function callGenerateContent(
  ...
  const ai = createGeminiClient();
  console.log(
    `[GeminiClient] Calling generateContent for model "${modelName}" with query: "${query}"`
  );
  ...
  if (textResponse === undefined) {
    console.warn(
      "[GeminiClient] generateContent response did not contain text:",
      result
    );
    throw new Error(
      "Failed to extract text from Gemini generateContent response."
    );
  }
...
async function callCountTokens(
  ...
  const ai = createGeminiClient();
  console.log(
    `[GeminiClient] Calling countTokens for model "${modelName}" with query: "${query}"`
  );
  ...
  if (response?.totalTokens === undefined) {
    console.warn(
      "[GeminiClient] countTokens response did not contain totalTokens:",
      response
    );
    throw new Error(
      "Failed to extract totalTokens from Gemini countTokens response."
    );
  }
...
async function callEmbedContent(
  ...
  const ai = createGeminiClient();
  console.log(
    `[GeminiClient] Calling embedContent for model "${modelName}" with query: "${query}"`
  );
  ...
  if (
    ...
    console.warn(
      "[GeminiClient] embedContent response did not contain embedding values:",
      response
    );
    throw new Error(
      "Failed to extract embedding values from Gemini embedContent response."
    );
  }
...
async function callGenerateImages(
  ...
  const ai = createGeminiClient();
  console.log(
    `[GeminiClient] Calling generateImages for model "${modelName}" with prompt: "${query}"`
  );
  ...
  if (!imageBytes) {
    console.warn(
      "[GeminiClient] generateImages response did not contain imageBytes:",
      response
    );
